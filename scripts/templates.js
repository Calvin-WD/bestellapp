...
export function getCategoryTemplate(indexCat, nameCat, dishesHtml) {
  return `<article class="category" aria-label="Categorie: ${nameCat}">
      <header id="categoryHeader-${indexCat}" class="category__header">
        <h2>${nameCat}</h2>
      </header>
      <div id="dishesHtmlContainer-${indexCat}" class="category__main">
      ${dishesHtml}
      </div>
    </article>`;
}
...
export function getDishTemplate(dish, buttonText, buttonClass, buttonState) {
  return `<article id="dish-${dish.id}" class="dish" aria-label="Dish: ${dish.name}" aria-describedby="dishDescription-${dish.id}">
      ...
      <div id="dishDescription-${dish.id}" class="dish__descriptionContainer">
        <div class="dish__description">
          ...
          <div class="dish__price">
            <span class="dish__price--value">${dish.price.toFixed(2)}€</span>
...
export function getBasketTemplate(
  ...
  return `<article
        id="${basketIdPrefix}-basket-id"
        class="basket"
        aria-label="the ${basketIdPrefix} basket">
        <header>
          <h2>Your Basket</h2>
        </header>
        <div class="basket__main">
          <section class="basket__dishesContainer ${scrollClass}">
          ${basketDishesHtml}
          </section>
...
function getDeleteButtonTemplate(dish, deleteButtonUpper, deleteButtonLower) {
  return `<button
          ...
          data-dish-id="${dish.id}"
          data-action="${BASKET_REMOVE_ITEM}"
          aria-label="removes the dish from the basket">
...
function getQuantityAdditionButtonTemplate(dish) {
  return `<button
            ...
            class="quantityBtn"
            data-dish-id="${dish.id}"
            data-action="${BASKET_ADD_AMOUNT}"
            aria-label="increases the amount of the dish in the basket by one">
              +
          </button>`;
}
...
function getQuantitySubtractionButtonTemplate(dish, quantitySubButton) {
  return `<button
            ...
            data-dish-id="${dish.id}"
            data-action="${BASKET_SUB_AMOUNT}"
            aria-label="decreases the amount of the dish in the basket by one">
              -
          </button>`;
}
