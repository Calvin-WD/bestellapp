...
export function getBasketDishTemplate(basketDish, deleteButtonUpper, deleteButtonLower, quantitySubButton) {
...
export function getBasketPriceTemplate(basket) {
  return `<table class="basket__table">
            <tr>
              <td>Subtotal</td>
              <td id="subTotal-id">${basket.summery.subTotal.toFixed(2)}€</td>
            </tr>
            <tr class="basket__table--padBottom">
              <td>Delivery Fee</td>
              <td>${basket.summery.fee.toFixed(2)}€</td>
            </tr>
            <tr>
              <th>Total</th>
              <th id="total-id">${basket.summery.total.toFixed(2)}€</th>
            </tr>
          </table>`;
}
...
function getDeleteButtonTemplate(basketDish, deleteButtonUpper, deleteButtonLower) {
  return `<button
          type="button"
          class="quantityBtn quantityBtn--delete js-delButton-${basketDish.id} ${deleteButtonUpper}${deleteButtonLower}"
          onclick="removeFromBasket(${basketDish.id}, 'delete')">
...
function getQuantityAdditionButtonTemplate(basketDish) {
  return `<button
            type="button"
            class="quantityBtn"
            onclick="changeBasketDishAmount(${basketDish.id}, 'addition')">
              +
          </button>`;
}

function getQuantitySubtractionButtonTemplate(basketDish, quantitySubButton) {
  return `<button
            id="subButton-${basketDish.id}"
            type="button"
            class="quantityBtn ${quantitySubButton}"
            onclick="changeBasketDishAmount(${basketDish.id}, 'subtraction')">
              -
          </button>`;
}
